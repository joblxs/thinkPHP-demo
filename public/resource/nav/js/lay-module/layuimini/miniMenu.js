...
layui.define(["element","laytpl" ,"jquery"], function (exports) {
    ...
    var miniMenu = {
        ...
        compileMenu: function(menu,isSub){
            var menuHtml = '<li lay-unselect {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} class="layui-nav-item menu-li {{d.childOpenClass}} {{d.className}}"  {{#if( d.id){ }}  id="{{d.id}}" {{#}}}> <a {{#if( d.href){ }} layuimini-href="{{d.href}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" {{#}}} href="#{{d.title}}">{{#if( d.icon){ }}  <i class="{{d.icon}}"></i> {{#}}} <span class="layui-left-nav">{{d.title}}</span></a>  {{# if(d.children){}} {{d.children}} {{#}}} </li>' ;
            if(isSub){
                menuHtml = '<dd lay-unselect class="menu-dd {{d.childOpenClass}} {{ d.className }}"> <a href="javascript:;"  {{#if( d.menu){ }}  data-menu="{{d.menu}}" {{#}}} {{#if( d.id){ }}  id="{{d.id}}" {{#}}} {{#if(( !d.child || !d.child.length ) && d.href){ }} layuimini-href="{{d.href}}" {{#}}} {{#if( d.target){ }}  target="{{d.target}}" data-filter="{{d.pid}}" data-tab="{{d.id}}" {{#}}}> {{#if( d.icon){ }}  <i class="{{d.icon}}"></i> {{#}}} <span class="layui-left-nav"> {{d.title}}</span></a> {{# if(d.children){}} {{d.children}} {{#}}}</dd>'
            }
        ...
        listen: function () {
            // 二级导航项添加点击事件
            $('.layuimini-menu-left').on('click', '.layui-nav-child dd', function() {
            ...
            $(document).ready(function() {
                // 使用 .on() 而不是 .one() 来确保每次点击都会触发事件
                $('a[href^="#"]').on('click', function(e) {
                    e.preventDefault();
                    var target = $(this).attr('href'); // 获取锚点
                    var targetElement = $(target); // 获取目标元素
            
                    // 确保目标元素存在
                    if (targetElement.length) {
                        // 计算目标元素相对于.myScrollContainer的顶部位置
                        var targetOffset = targetElement.offset().top - $('.main').offset().top + $('.main').scrollTop();
